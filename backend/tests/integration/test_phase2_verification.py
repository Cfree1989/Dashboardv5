...
def test_analytics_events_list(client, token):
    ...
    submit_resp = client.post('/api/v1/submit', data=data, content_type='multipart/form-data')
    assert submit_resp.status_code == 201
    job = submit_resp.get_json()
    job_id = job['id']

    resp = client.get('/api/v1/analytics/events', headers={'Authorization': f'Bearer {token}'})
    assert resp.status_code == 200
    events = resp.get_json()
    assert any(evt['event_type'] == 'JobCreated' and evt['job_id'] == job_id for evt in events)
